...
#define LCD_ADDR "127.0.0.1"
#define LCD_PORT  15000 
...
void SendMessageToLCD(char *message)
{
    ...
    s_addr.sin_addr.s_addr = inet_addr(LCD_ADDR);
    s_addr.sin_port        = htons(LCD_PORT);

    status = sendto(LCDsockid, 
                    message, size, 0,
...
void CFE_PSP_1HzLoop(void)
{
   ...
   */
   for ( ;; )
   {
      ...
      cpuPercent = loadavg * 100;

      sprintf(MsgString, "T:%04ds CPU:%03d%%",seconds,cpuPercent);
      SendMessageToLCD(MsgString);
   }
}
...
int main(int argc, char *argv[])
{
   ...
   ** Open Socket
   */
   if ( (LCDsockid = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
   {
      printf("Error, could not open socket to LCD!\n");
   }
